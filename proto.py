...
from PyQt5.QtWidgets import (
    ...
    QAbstractItemView, QInputDialog, QSplitter, QGraphicsOpacityEffect, QDialog, QSizePolicy    # ✅ Correction ici
)
...
class SortableHeaderView(QHeaderView):
    def __init__(self, orientation, table, parent=None):
        ...
        self.sectionClicked.connect(self.sort_table)  # ✅ Correction


    def sort_table(self, logical_index):
        """Tri les colonnes lorsqu'on clique sur l'en-tête."""
        if logical_index < 0:
            print("⚠️ [DEBUG] Index de colonne invalide, tri annulé.")
            return

        print(f"🟢 [DEBUG] Clic sur la colonne {logical_index}")  # Debug

        order = self.sortIndicatorOrder()
        order_str = "Ascendant" if order == Qt.AscendingOrder else "Descendant"
        print(f"🔄 [DEBUG] Tri en cours, ordre : {order_str}")

        parent = self.table.parent()
        if parent and hasattr(parent, "sort_table_data"):
            print(f"✅ [DEBUG] Envoi du tri à BookingApp pour la colonne {logical_index}")  # Debug
            parent.sort_table_data(logical_index, order)
            self.setSortIndicator(logical_index, order)  # ✅ Met à jour l'indicateur visuel de tri
        else:
            print(f"❌ [DEBUG] BookingApp ne contient pas sort_table_data()")  # Problème possible


    def sort_by_dropdown(self, column, order, custom_order):
        """Trie une colonne contenant des QComboBox en fonction d'un ordre personnalisé."""
        
        def get_order_value(row):
            """Récupère la position du texte affiché dans le QComboBox ou une valeur élevée s'il est absent."""
            widget = self.table.cellWidget(row, column)
            if isinstance(widget, QComboBox):
                value = widget.currentText()
                return custom_order.index(value) if value in custom_order else len(custom_order)
            return len(custom_order)  # Place les valeurs inconnues en bas du tri

        # Récupération des indices de lignes et des valeurs de tri
        rows = list(range(self.table.rowCount()))
        sorted_rows = sorted(rows, key=get_order_value, reverse=(order == Qt.DescendingOrder))
        ...
        for row in sorted_rows:
            ...
            for col in range(self.table.columnCount()):
                ...
                if isinstance(widget, QComboBox):
                    row_data.append(widget.currentText())  # ✅ Récupère le texte du QComboBox
                elif item:
                    row_data.append(item.text())  # ✅ Récupère le texte normal
                else:
                    row_data.append("")
            sorted_data.append(row_data)

        # 🔄 Réinitialiser et re-remplir la table après tri
        self.table.setRowCount(0)
        for row_data in sorted_data:
            row_pos = self.table.rowCount()
            self.table.insertRow(row_pos)
            for col, value in enumerate(row_data):
                if col == self.get_statut_column_index() or col == self.get_formule_column_index():
                    combo = QComboBox()
                    combo.addItems(custom_order)
                    combo.setCurrentText(value)
                    self.table.setCellWidget(row_pos, col, combo)
                else:
                    self.table.setItem(row_pos, col, QTableWidgetItem(value))

        # ✅ Mettre à jour les couleurs après le tri
    ...
    def export_to_excel(self, file_path):
        print("[DEBUG] Début export Excel")
        data = []
        for row in range(self.table.rowCount()):
            row_data = [self.table.item(row, col).text() if self.table.item(row, col) else "" for col in range(self.table.columnCount())]
            data.append(row_data)

        df = pd.DataFrame(data, columns=["Date", "Statut", "Cachet", "Formule"])
        df.to_excel(file_path, index=False)
        print("[DEBUG] Export Excel terminé.")

    def debug_click(self, logical_index):
        print(f"✅ [DEBUG] En-tête cliqué : Colonne {logical_index}")
...
class DraggableTableWidget(QTableWidget):
    ...
    def dropEvent(self, event):
        ...
        rows_data = [[self.item(row, col).text() if self.item(row, col) else "" for col in range(self.columnCount())] for row in selected_rows]
...
class BookingApp(QMainWindow):
    def __init__(self):
        ...
        # Remplacement de l'en-tête de la table pour le tri
        self.table.setHorizontalHeader(SortableHeaderView(Qt.Horizontal, self.table))
    ...
    def sort_table_data(self, column, order):
        """Trie les colonnes correctement en fonction de leur type."""
        print(f"🟢 [DEBUG] Tri demandé sur la colonne {column}, ordre {order}")  # Debug

        if column == self.get_date_column_index():  # 📅 DATE
            self.sort_by_date(column, order)
        elif column == self.get_statut_column_index():  # 📌 STATUT (QComboBox)
            self.sort_by_dropdown(column, order, ["Nouveau", "Mail envoyé", "Échange Tel.", "Full", "Laisse tomber", "Let's Go"])
        elif column == self.get_cachet_column_index():  # 💰 CACHET (Valeurs numériques)
            self.sort_by_numeric(column, order)
        elif column == self.get_formule_column_index():  # 📜 FORMULE (QComboBox)
            self.sort_by_dropdown(column, order, ["Standard", "VIP", "Backstage"])
        else:
            self.table.sortItems(column, order)  # Tri standard

        # ✅ Mettre à jour les couleurs après le tri
        for row in range(self.table.rowCount()):
            self.update_row_color(row)

        # ✅ Forcer le rafraîchissement de la table après le tri
        self.table.viewport().update()


    def sort_by_numeric(self, column, order):
        """Trie les valeurs numériques correctement, y compris celles avec des virgules."""
        def numeric_value(row):
            item = self.table.item(row, column)
            try:
                return float(item.text().replace(",", ".")) if item else 0
            except ValueError:
                return 0

        self.sort_rows(column, order, numeric_value)


    def sort_by_combobox(self, column, order):
        """Trie une colonne contenant un QComboBox en fonction du texte affiché."""
        def combobox_value(row):
            widget = self.table.cellWidget(row, column)
            if isinstance(widget, QComboBox):
                return widget.currentText()  # ✅ Récupère le texte affiché du menu déroulant
            return ""

        self.sort_rows(column, order, combobox_value)


    def sort_by_date(self, column, order):
        """Trie les dates correctement au format YYYY-MM-DD."""
        def date_value(row):
            item = self.table.item(row, column)
            if item:
                try:
                    return QDate.fromString(item.text(), "yyyy-MM-dd")
                except ValueError:
                    return QDate()  # Valeur minimale si la date est invalide
            return QDate()

        self.sort_rows(column, order, date_value)

    def sort_by_numeric(self, column, order):
        """Trie les valeurs numériques correctement."""
        def numeric_value(row):
            item = self.table.item(row, column)
            try:
                return float(item.text().replace(",", ".")) if item else 0
    ...
    def sort_by_dropdown(self, column, order, custom_order):
        """Trie une colonne en fonction d'un ordre personnalisé."""
        def dropdown_value(row):
            item = self.table.cellWidget(row, column)
            if isinstance(item, QComboBox):
                return custom_order.index(item.currentText()) if item.currentText() in custom_order else len(custom_order)
            return len(custom_order)

        self.sort_rows(column, order, dropdown_value)

    def sort_rows(self, column, order, key_function):
        """Trie les lignes de la table en fonction d'une clé personnalisée (textes et QComboBox)."""
        rows = list(range(self.table.rowCount()))
        rows.sort(key=key_function, reverse=(order == Qt.DescendingOrder))
        ...
        for row_data in sorted_data:
            ...
            for col, value in enumerate(row_data):
                if col == self.get_statut_column_index() or col == self.get_formule_column_index():
                    combo = QComboBox()
                    combo.addItems(["Nouveau", "Mail envoyé", "Échange Tel.", "Full", "Laisse tomber", "Let's Go"])
                    combo.setCurrentText(value)
                    self.table.setCellWidget(row_pos, col, combo)
                else:
                    self.table.setItem(row_pos, col, QTableWidgetItem(value))
    ...
    def get_statut_column_index(self):
        ...
        for col in range(self.table.columnCount()):
            header_item = self.table.horizontalHeaderItem(col)
            if header_item and header_item.text().strip().lower() == "statut":
                return col
        print("⚠️ [DEBUG] Colonne 'Statut' introuvable ! Vérifie le nom des en-têtes.")
        return -1  # Évite les erreurs en cas de problème
    ...
    def sort_columns(self):
        """Trie la table en fonction du critère sélectionné dans le menu déroulant."""
        index = self.sort_dropdown.currentIndex()

        if index == 0:
            return  # Ne rien faire si l'option "Trier par..." est sélectionnée

        # Définition des indices des colonnes
        columns_mapping = {
            1: 0,  # "Date (A-Z)"
            2: 0,  # "Date (Z-A)"
            3: 1,  # "Statut (A-Z)"
        ...
        column = columns_mapping.get(index, 0)
        order = Qt.AscendingOrder if index % 2 == 1 else Qt.DescendingOrder
    ...
    def update_status_value(self, row, col, combo_box):
        """Stocke la valeur du choix sélectionné sans casser le QComboBox et met à jour la couleur."""
        ...
        if item:
            ...
            self.table.setItem(row, col, QTableWidgetItem(selected_value))

        # Met à jour la couleur de la ligne après changement de statut
        self.update_row_color(row)
    ...
    def add_status_combobox(self, row):
        """Ajoute un menu déroulant à la colonne Statut et met à jour les couleurs."""
        ...
        for row in range(self.table.rowCount()):
            ...
            # Connecte le changement de statut à la mise à jour des couleurs
            combobox.currentIndexChanged.connect(lambda _, r=row: self.update_row_color(r))
            
            self.table.setCellWidget(row, statut_col, combobox)
    ...
    def get_formule_column_index(self):
        ...
        for col in range(self.table.columnCount()):
            header_item = self.table.horizontalHeaderItem(col)
            if header_item and header_item.text().strip().lower() == "formule":
                return col
        print("⚠️ [DEBUG] Colonne 'Formule' introuvable ! Vérifie le nom des en-têtes.")
        return -1  # Évite les erreurs en cas de problème
    ...
    def add_row(self):
        """Ajoute une nouvelle ligne avec des listes déroulantes pour les colonnes 'Statut' et 'Formule'."""
